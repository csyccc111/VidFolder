...
[![Version](https://img.shields.io/badge/version-v0.9.0-2ea44f)](https://github.com/csyccc111/VidFolder/releases)
...
**依赖**：`ffmpeg` 与 `ffprobe`。应用会自动探测（应用内版本 → 手动指定 → 系统 PATH → 常见安装位置）；缺失时可**在应用内一键下载**（固定版本 + SHA-256 校验，见下文「依赖自动管理」）。缺失时仍可打开应用、浏览已有缓存；新视频的封面或元信息将无法生成。
...
- **智能探测**：按 应用内版本 → 手动指定路径 → 系统 PATH → 常见安装位置（C:\ffmpeg、Program Files、Scoop、Chocolatey、WinGet）顺序探测，结果持久化，失效自动重探
- **按需下载**：依赖缺失时状态栏提示"下载 ffmpeg"，点击后在应用内下载固定版本的 [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds) win64 构建（约 76 MB），带下载进度、速度显示与取消/重试；下载完成后经 SHA-256 校验再解压启用
- **安全**：下载包与内置 SHA-256 清单比对，不一致立即清理并明确报错；解压路径双重防 zip-slip 校验
- **版本切换**：详情面板可查看各工具来源与版本；支持"恢复系统版本 / 启用应用内版本"切换与手动指定 ffmpeg/ffprobe 路径
- 不自动静默下载（始终需要用户确认）；不做版本自动更新

**GPL 声明**：应用内下载的 ffmpeg 二进制来自 BtbN/FFmpeg-Builds（GPL v3 构建）。本项目不随安装包分发该二进制，仅在用户明确确认后下载安装到应用数据目录；来源与版本信息见应用内依赖管理面板。
...
UI 回归冒烟脚本（真实 Electron + ffmpeg，覆盖各版本功能）：`scripts/smoke-dom.cjs`、`scripts/smoke-v08.cjs`、`scripts/smoke-v09.cjs`；依赖下载端到端验证：`scripts/test-download-e2e.cjs`（真实网络下载约 76MB）。
...
- 应用内下载仅支持 Windows x64；不做版本自动更新（可重复点击下载覆盖为清单版本）。
...
- `cache/`：封面缩略图
- `vendor/ffmpeg/`：应用内下载的 ffmpeg/ffprobe（含版本记录 version.json，可在依赖面板切换或停用）
